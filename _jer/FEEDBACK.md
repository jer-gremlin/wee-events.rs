...
- [x] **[ARCH/WONTFIX]** `src/service.rs:51-93` — `ServiceError`'s five `From` impls audited; complaint declined. The "cycles" all converge to the same value (e.g. `EncodeError → CodecError → ServiceError::Codec(Encode(e))` and the direct `EncodeError → ServiceError::Codec(Encode(e))` yield identical results — they're ergonomic shortcuts for `?`, not bug-causing). The "asymmetric `From<RenderError<DeserializeJsonError>>`" half is stale: `DeserializeJsonError` was deleted earlier on this branch, leaving only the matching `From<RenderError<DecodeError>>` impl. Suggested rewrite (one `Custom(Box<dyn Error>)` variant) would erase the typed `Rejection` / `Store(E)` / `Codec(CodecError)` discrimination that adapters depend on — `wee-events-restate` literally pattern-matches each variant to map onto Restate's terminal/transient error model (`error.rs:53-58`, `lib.rs:148-159`). Cleanup applied: removed a stale doc comment that referenced the now-deleted `EH: From<crate::Error>` bound and clarified that the `From<crate::Error> for ServiceError<E>` impl is the canonical `EL: From<crate::Error>` gateway when `ServiceError` plays the EL role.
- [x] **[ARCH]** `src/service_builder.rs:453-475` — `build_raw` **deleted**. The `#[doc(hidden)] pub fn build_raw` was a near-duplicate of `build` that skipped the factory trait bound. Workspace grep confirmed zero callers outside `build` itself — the "adapter escape hatch" was hypothetical, but technically-public so any consumer could reach in and bypass the bound. Inlined the body into `build`, removed the unused function. No behaviour change.
- [x] **[BUG]** `src/service_builder.rs:299-300` — Dual-path `crate::Error → EH` ambiguity **sealed**. Dropped the `EH: From<crate::Error>` bound from `BuiltService`'s impl block; `EH` only needs `From<EL>` now. `crate::Error` enters the error world through one canonical gateway: `EL: From<crate::Error>` then `EH: From<EL>`. The one direct-path callsite (`execute`'s factory failure) re-routed to `.map_err(EL::from).map_err(EH::from)`. Mirrored in the macro-emitted dispatch bound (`service_macro.rs` no longer emits `+ From<::wee_events::Error>`). Module-level + `FactoryBridge` docs updated to state the rule. Behaviour change is only visible when callers had previously written diverging `From<crate::Error> for EL` and `From<crate::Error> for EH` impls — in practice nobody had; in-tree `EL = EH = ServiceError<...>` and both routes converged. The framework no longer *permits* the divergence.
